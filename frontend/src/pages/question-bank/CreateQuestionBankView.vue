...
function buildProblemPayload(problem: ProblemDraft): ProblemPayload | null {
  ...
  if (problem.type === 3) {
    ...
    return hint
      ? { type: 3, content, answer, hint }
      : { type: 3, content, answer }
  }
  ...
  if (problem.type === 2) {
    ...
    return hint
      ? { type: 2, content, choices, answer, hint }
      : { type: 2, content, choices, answer }
  }
  ...
  if (problem.type === 4) {
    ...
    return hint
      ? { type: 4, content, choices: pair, answer, hint }
      : { type: 4, content, choices: pair, answer }
  }
  return hint
    ? { type: 1, content, choices, answer, hint }
    : { type: 1, content, choices, answer }
}
...
<template>
  <section class="page">
    ...
    <header class="page-head">
      <div>
        <div class="eyebrow">新建题库</div>
        <h1>创建你的题库</h1>
        <p>填写题库信息并录入题目，支持单选、多选、填空和判断题。</p>
      </div>
      <div class="page-actions">
        <Button label="返回题库" severity="secondary" text size="small" @click="router.push({ name: 'question-bank' })" />
      </div>
    </header>
    ...
    <section v-else class="vtix-panel import-panel">
      ...
      <div class="vtix-panel__content import-content">
        ...
        <div class="import-tip">
          函数返回格式示例：{ title, type, choices, answer, hint }。支持 title 或 content 作为题干。<br>
          type：1 单选、2 多选、3 填空、4 判断。<br>
          answer：单选/判断为数字下标（0 开始），多选为下标数组，填空为字符串。
        </div>
        <label class="field">
          <span>解析函数</span>
...
.import-panel :deep(.p-inputtextarea) {
